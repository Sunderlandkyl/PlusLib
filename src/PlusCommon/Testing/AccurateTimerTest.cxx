...
// Test if no thread hang occur when using vtkIGSIOAccurateTimer
...
void* timerTestThread( vtkMultiThreader::ThreadInfo *data )
{
  ...
  while ( !gStopRequested )
  {
    ...
    vtkIGSIOAccurateTimer::Delay(delaySec);
...
int main(int argc, char **argv)
{
  ...
  {
#ifdef PLUS_TEST_HIGH_ACCURACY_TIMING
    ...
    return EXIT_FAILURE;
#else
    LOG_INFO("There were " << gNumberOfDelayErrors << " timer errors (more than " << gMaxDelayErrorSec << " sec difference in intended and actual delay). Testing of high-accuracy timing has been disabled, so timer errors are not reported as test failures.");
#endif
  }
